...
def ssfinder(k1_val, k2_val, theta1_val, theta2_val):
    ...
    for InitGuess in InitGuesses:
        ...
        if np.all(output > 0.04) and np.linalg.norm(fvec) < 1e-10 and intflag == 1 and not unstable:
...
def fobj(solution):
    ...
    t_max = 5e4
    n_points = 10000  # Adjust resolution as needed
...
#               args=(params,),
    ...
    print(f"Integral over time up to t = {t_max}: {integral_val:.6f}")
